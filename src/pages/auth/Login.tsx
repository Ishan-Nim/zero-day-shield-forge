...
const Login = () => {
  ...
  const onSubmit = async (values: z.infer<typeof loginSchema>) => {
    ...
    try {
      ...
      // Navigation is handled in AuthContext to ensure it happens after state updates
    } catch (error: any) {
    ...
    <div className="flex min-h-screen items-center justify-center bg-gray-50 px-4 py-12 sm:px-6 lg:px-8">
      <Card className="w-full max-w-md">
        <CardHeader className="space-y-2 text-center">
          ...
          <CardDescription>
            Enter your email and password to access your security dashboard
          </CardDescription>
        </CardHeader>
        <CardContent>
          {loginError && (
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="you@example.com" autoComplete="email" disabled={isLoading} {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
